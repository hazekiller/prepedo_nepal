...
export default function CreatePartnerPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEdit ? API_ENDPOINTS.updatePartner(partnerId) : API_ENDPOINTS.createPartner;
      ...
      if (data.success) {
        Alert.alert('Success', `Partner ${isEdit ? 'updated' : 'created'} successfully`, [
          {
            text: 'OK',
            onPress: () => router.back(),
          },
        ...
        Alert.alert('Error', data.message || `Failed to ${isEdit ? 'update' : 'create'} partner`);
      }
    } catch (error) {
      console.error('Submit partner error:', error);
      Alert.alert('Error', `Failed to ${isEdit ? 'update' : 'create'} partner`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isEdit ? 'Edit' : 'Create'} Partner</Text>
        <View style={{ width: 60 }} />
      </View>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.formGroup}>
            ...
            <Text style={styles.helpText}>
              {formData.is_active
                ? 'Partner is visible on the website'
                : 'Partner is hidden'}
            </Text>
          </View>
